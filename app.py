...
# PDF text extraction and OCR helper functions are imported from qc_logic.py
...
class PDFQCApp(tk.Tk):
    ...
    def parse_table_row(line):
        return qc_logic.parse_table_row(line)

    @staticmethod
    def normalize_word_token(text):
        return qc_logic.normalize_word_token(text)
    ...
    def _pos_numeric_value(pos):
        return qc_logic._pos_numeric_value(pos)
    ...
    def _normalize_token_variants(token_variants):
        return qc_logic._normalize_token_variants(token_variants)
    ...
    def _match_callout_on_page(tokens, words):
        return qc_logic._match_callout_on_page(tokens, words)

    @staticmethod
    def _numeric_from_text(text):
        return qc_logic._numeric_from_text(text)
    ...
    def _pick_candidate(candidates, expected_length, prefer_max=False, tolerance=0.5):
        return qc_logic._pick_candidate(candidates, expected_length, prefer_max, tolerance)
    ...
    @staticmethod
    def tokenize(text):
        return qc_logic.tokenize(text)

    @staticmethod
    def contains_subsequence(tokens, subseq):
        return qc_logic.contains_subsequence(tokens, subseq)

    @staticmethod
    def extract_numeric_values(text):
        return qc_logic.extract_numeric_values(text)
    ...
    def format_length(value):
        return qc_logic.format_length(value)
    ...
    def _on_spellcheck_complete(self, job_id, issues):
        ...
        self._spellcheck_thread = None
        self._update_upload_button_state()
# Plumber helpers and table snippet cleaners are imported from qc_logic.py
